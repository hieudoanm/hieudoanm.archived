...
const Openings: FC<{ makeMoves: (pgn: string) => void }> = ({
  ...
  return (
    <div className='border-base-content overflow-hidden rounded-xl border'>
      <table className='table'>
        ...
        {openings.map(({ eco = '', name = '', pgn = '' }: Partial<Opening>) => {
          return (
            <tr
              key={`${eco}-${name}`}
              className={`border-base-content border-t ${selectedPgn === pgn ? 'bg-primary text-secondary' : ''}`}>
              <td
                onClick={() => {
                  setSelectedPgn(pgn);
                  makeMoves(pgn);
        ...
        <div className='h-full p-4 md:p-8'>
          <div className='grid h-full grid-cols-1 gap-2 md:grid-cols-7 md:gap-4'>
            <div className='col-span-1 flex items-center md:col-span-3'>
              <div className='border-base-content w-full overflow-hidden rounded-xl border'>
