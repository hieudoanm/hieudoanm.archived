...
export class AccuracyService {
  ...
  private buildAccuracyByResultsQuery(
    ...
    const averageQuery = `SELECT ${averageClause} FROM public."Game" as g WHERE ${whereClause} AND ${clause}`;
  ...
  private buildAverageAccuracyQuery({
    ...
    const averageQuery = `SELECT ${averageClause} FROM public."Game" as g WHERE ${whereClause}`;
    const sql: Prisma.Sql = Prisma.raw(averageQuery);
    return sql;
  }
  ...
  private buildAverageAccuracyByPeriods({
    ...
    const query = `SELECT ${extractClause}, ${averageClause} FROM public."Game" as g WHERE ${whereClause} ${groupByClause}`;
