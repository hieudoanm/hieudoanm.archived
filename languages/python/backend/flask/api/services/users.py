...
import uuid
from api.libs.postgres import get_connection
...
def create_user(user):
    ...
    CREATE_USER_QUERY = CREATE_USER % (new_id, username, password)
    ...
    cursor.close()
    connection.close()
    return {"id": new_id, "username": new_username, "password": new_password}
...
def update_user(user_id, user):
    ...
    username = user.get("username", "")
    password = user.get("password", "")
    UPDATE_USER_QUERY = UPDATE_USER % (username, password, user_id)
    ...
    cursor.close()
    connection.close()
    return {"id": user_id, "username": updated_username, "password": updated_password}
